...
- **Familienzweige:** `buildFamilies()` erkennt Wurzeln automatisch:
  ...
  **Jacobsdorf (Pomm)**, „Großenhagen"/„Jannike…" → **Großenhagen (Pomm)**;
  ...
  Solche Vorfahren oberhalb bleiben in den Daten, werden aber **nicht
  gezeichnet**: die Vorfahren-Familie (`BRANCH_ANCESTORS`, erkannt daran,
  dass ein Elternteil einer festen Wurzel darin liegt) wird aufgelöst und
  ihre Personen jeder Linie darunter zugeschlagen (`assigned` + Set
  `ancestors`); `App.familySubset` lässt `ancestors` aus (sonst stünde in
...
(von Fächer, Stammtafel, Gotha, App und Karte gemeinsam genutzt) berechnet
die Zweige wie immer und ersetzt dann **den Zweig der Person durch ihren
Teilbaum** (`rootId` bleibt die Zweig-Wurzel — Umschalter und gespeicherte
Auswahl funktionieren weiter; `root`/`assigned`/`size` sind der Teilbaum,
`name` bekommt „· Stammperson …"). Die anderen Zweige bleiben vollständig;
...
Filter, keine Lücke). Nicht persistent. Nutzungsereignis `temp_root`.
